...
const UserSellers = () => {
  ...
  const getSellers = async () => {
    try {
      ...
      return sellers || []; // Ensure empty array if no sellers are returned
    } catch (err) {
  ...
  const handleBanToggle = async (id, currentStatus) => {
    const confirmAction = window.confirm(
      `Are you sure you want to ${currentStatus === "banned" ? "unban" : "ban"} this user?`
    );
    if (!confirmAction) return;

    const updatedSellers = data.map((user) =>
      user._id === id
        ? { ...user, role: currentStatus === "banned" ? "seller" : "banned" }
        : user
    );

    queryClient.setQueryData(["seller"], updatedSellers);  
    try {
      const response = await banUsers(token, id, currentStatus === "banned" ? "seller" : "banned");
      if (response) {
      refetch(); 
                    ...
                    <tr key={_id}>
                      ...
                      <td>
                        <span
                          ...
                          style={role === "seller" ? { color: "#fff", backgroundColor: "#1A2B49" } : {}}
                        >
                          {role}
                        </span>
                      </td>
                      ...
                      <td>
                        ...
                        <button
                          className={`btn ${role === "banned" ? "btn-success" : "btn-danger"} me-2`}
